...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // 1. Ambil statistik dari API Route
        const statsRes = await fetch("/api/admin/stats");
        
        // Proteksi jika response bukan JSON (mencegah error <!DOCTYPE HTML)
        if (!statsRes.ok) {
        ...
        setStats({
          totalSurat: statsData.totalSurat || 0,
          totalUser: statsData.totalUser || 0,
          totalLogs: statsData.totalLogs || 0,
          taskDone: statsData.taskDone || 0
        });
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F4F7FE] gap-4">
        ...
        />
        <p className="text-slate-500 font-bold animate-pulse">Menghubungkan ke Database...</p>
      </div>
    );
  }
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          <div className="mb-10">
            ...
            <h1 className="text-4xl font-black text-slate-900 leading-tight">Halo, {user.name ? user.name.split(' ')[0] : 'Admin'}! 👋</h1>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              {[
                ...
                { label: "Log Aktivitas", val: stats.totalLogs, desc: "Tabel: logs", icon: <Activity />, color: "bg-orange-500" },
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-10">
            <motion.div whileHover={{ y: -10 }} variants={itemVars} initial="hidden" animate="show">
              <Card className="rounded-[32px] border-none shadow-sm hover:shadow-2xl transition-all cursor-pointer group h-full" onClick={() => router.push("/admin/logs")}>
                ...
                <CardContent>
                  <p className="text-slate-500 text-sm leading-relaxed mb-8">Pantau aktivitas sistem dari tabel <b>logs</b> secara real-time.</p>
            ...
            <motion.div whileHover={{ y: -10 }} variants={itemVars} initial="hidden" animate="show">
              <Card className="rounded-[32px] border-none shadow-sm hover:shadow-2xl transition-all cursor-pointer group h-full" onClick={() => router.push("/admin/pengguna")}>
                ...
                <CardContent>
                  <p className="text-slate-500 text-sm leading-relaxed mb-8">Manajemen data <b>users</b>. Tambah, edit, atau hapus akses pengguna.</p>
            ...
            <motion.div whileHover={{ y: -10 }} variants={itemVars} initial="hidden" animate="show">
              <Card className="rounded-[32px] border-none shadow-sm hover:shadow-2xl transition-all cursor-pointer group h-full" onClick={() => router.push("/admin/arsip")}>
                ...
                <CardContent>
                  <p className="text-slate-500 text-sm leading-relaxed mb-8">Akses data tabel <b>surat</b>. Verifikasi hasil AI dan manajemen arsip.</p>
